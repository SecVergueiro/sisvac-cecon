...
const ACTION_CARDS = [
  { color:'accent', badge: true,  icon:'check-cal', title:'Aprovar solicita├º├Áes',  desc:'Pedidos aguardando sua an├ílise', href:'/aprovacoes'   },
  { color:'navy',   badge: false, icon:'cal-plus',  title:'Nova solicita├º├úo',      desc:'Registrar f├®rias para um servidor', href:'/ferias'    },
  { color:'green',  badge: false, icon:'users',     title:'Consultar servidores',  desc:'Ver saldos, hist├│rico e dados',  href:'/servidores'   },
  ...
  { color:'purple', badge: false, icon:'file',      title:'Gerar relat├│rio',       desc:'Exportar PDF por setor ou per├¡odo', href:'/relatorios' },
  { color:'red',    badge: false, icon:'clock',     title:'Saldos em risco',       desc:'Servidores pr├│ximos ao vencimento', href:'/servidores' },
]
...
export default function DashboardClient({ kpis, aprovacoes, emGozo }: Props) {
  ...
  const KPIS = [
    ...
    { label:'Aprova├º├Áes pendentes', value: kpis.pendentesAprovacao,    delta: kpis.pendentesAprovacao > 0 ? 'Aguardando' : 'Em dia', type: kpis.pendentesAprovacao > 0 ? 'warn' : 'up' },
    { label:'Aprova├º├Áes este m├¬s',  value: kpis.aprovacoesMesAtual,    delta:'M├¬s atual',                  type:'up'     },
    { label:'Em risco de perda',    value: kpis.servidoresEmRisco,     delta: kpis.servidoresEmRisco > 0 ? 'Aten├º├úo' : 'Sem risco', type: kpis.servidoresEmRisco > 0 ? 'down' : 'up' },
  ]
      ...
      <div className="shell">
        ...
        <div className="main">
          <div className="topbar">
            <div>
              <div className="tb-title">In├¡cio</div>
              <div className="tb-sub">{greeting} ┬À {month.charAt(0).toUpperCase() + month.slice(1)}</div>
            </div>
            <div className="tb-right">
              <div className="tb-search">
                <Icon t="search" />
                <input placeholder="Buscar servidorÔÇª" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
          ...
          <div className="content">
            <div className="rh-greeting">
              <h2>O que voc├¬ precisa fazer hoje?</h2>
              <p>Selecione uma a├º├úo abaixo ou acompanhe os indicadores da funda├º├úo</p>
            </div>
            ...
            {kpis.servidoresEmRisco > 0 && (
              <div className="alert-strip">
                ...
                <div className="alert-text">
                  <strong>{kpis.servidoresEmRisco} servidor{kpis.servidoresEmRisco !== 1 ? 'es' : ''} em risco de perda de f├®rias</strong>
                  <p>Saldos com vencimento nos pr├│ximos 60 dias sem solicita├º├úo em aberto</p>
                </div>
            ...
            <div className="two-col">
              <div className="card">
                <div className="card-head">
                  <h3>Aprova├º├Áes recentes</h3>
                  <button className="btn" onClick={() => router.push('/aprovacoes')}>Ver todas</button>
                </div>
                <table className="mt">
                  <thead><tr><th>Servidor</th><th>Per├¡odo</th><th>Status</th></tr></thead>
                  <tbody>
                    {aprovacoes.length === 0 ? (
                      <tr className="empty-row"><td colSpan={3}>Nenhuma solicita├º├úo recente</td></tr>
                    ) : aprovacoes.map((a, i) => (
                      <tr key={i}>
