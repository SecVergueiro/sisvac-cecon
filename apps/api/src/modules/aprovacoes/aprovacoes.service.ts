...
export class AprovacoesService {
  ...
  async aprovar(
    ...
    if (
      solicitacao.tipo_afastamento !== TipoAfastamento.LICENCA_ESPECIAL &&
      solicitacao.id_saldo_ferias
    ) {
      ...
      if (saldo) {
        const novoUtilizados =
          (saldo.dias_utilizados || 0) + solicitacao.dias_solicitados;
        const novoDisponiveis =
          (saldo.dias_disponiveis || 0) - solicitacao.dias_solicitados;
        ...
        if (updErr)
          throw new BadRequestException("Erro ao descontar o saldo de férias.");
      }
    } else if (
      solicitacao.tipo_afastamento === TipoAfastamento.LICENCA_ESPECIAL &&
      solicitacao.id_saldo_licenca
    ) {
      ...
      if (saldoLic) {
        await supabase
          ...
          .update({
            dias_utilizados:
              (saldoLic.dias_utilizados || 0) + solicitacao.dias_solicitados,
            dias_disponiveis:
              (saldoLic.dias_disponiveis || 0) - solicitacao.dias_solicitados,
          })
          .eq("id", saldoLic.id);
      }
    }
    ...
    const { error: updSolErr } = await supabase
      .from("solicitacoes")
      .update(updatePayload)
      .eq("id", id);
    if (updSolErr)
      throw new BadRequestException(
        "Falha ao atualizar o status da aprovação.",
      );
  ...
  async negar(id: string, dto: NegarDto, userId: string) {
    ...
    if (!solicitacao || solicitacao.status !== StatusSolicitacao.PENDENTE) {
      throw new BadRequestException(
        "Apenas solicitações pendentes podem ser negadas.",
      );
    }
    ...
    const { error } = await supabase
      ...
      .eq("id", id);
    if (error)
      throw new BadRequestException("Não foi possível negar a requisição.");
