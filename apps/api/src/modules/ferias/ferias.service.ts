...
import { StatusSolicitacao, TipoAfastamento, TipoFracionamento } from '@sisvac/types';
...
export class FeriasService {
  ...
  async criarSolicitacao(dto: CriarFeriasDto, userId: string) {
    ...
    if (dto.tipoAfastamento !== TipoAfastamento.LICENCA_ESPECIAL) {
      if (!dto.idSaldoFerias) {
        throw new BadRequestException('ID do saldo de férias é obrigatório para Férias.');
      }
      ...
      switch (dto.tipoFracionamento) {
        case TipoFracionamento.INTEGRAL:
          if (dto.diasSolicitados !== 30) throw new BadRequestException('Férias integrais exigem 30 dias.');
          break;
        case TipoFracionamento.QUINZE_QUINZE:
          if (dto.diasSolicitados !== 15) throw new BadRequestException('Fração de 15/15 exige 15 dias.');
          break;
        case TipoFracionamento.DEZ_VINTE:
          if (dto.diasSolicitados !== 10 && dto.diasSolicitados !== 20) {
            throw new BadRequestException('Fração de 10/20 exige blocos de 10 ou 20 dias.');
          }
          break;
        case TipoFracionamento.DEZ_DEZ_DEZ:
          if (dto.diasSolicitados !== 10) throw new BadRequestException('Fração de 10/10/10 exige blocos de 10 dias.');
          break;
      }
      ...
      if (solicitacoesAnteriores && solicitacoesAnteriores.length > 0) {
        const fracionamentoAnterior = solicitacoesAnteriores[0].tipo_fracionamento;
        if (fracionamentoAnterior !== dto.tipoFracionamento) {
          throw new BadRequestException(`As frações devem manter o padrão selecionado no primeiro período (${fracionamentoAnterior}).`);
        }
      }
    } else {
      if (!dto.idSaldoLicenca) {
        throw new BadRequestException('ID do saldo de licença é obrigatório para Licença Especial.');
      }
      if (dto.diasSolicitados > 90) {
        throw new BadRequestException('Licença especial tem o máximo de 90 dias.');
      }
    }
    ...
    if (overlaps && overlaps.length > 0) {
      throw new BadRequestException('Já existe uma solicitação ou período agendado que coincide com estas datas.');
    }
    ...
    if (insertError) {
      throw new BadRequestException(`Erro ao criar solicitação: ${insertError.message}`);
    }
  ...
  async cancelar(id: string, userId: string, userRole: any) {
    ...
    if (solErr || !sol) throw new NotFoundException('Solicitação não encontrada.');

    // Regra de Ownership (Apenas dono ou Gestores podem cancelar)
    if (sol.solicitado_por !== userId && userRole !== 'ADMIN' && userRole !== 'RH') {
      throw new ForbiddenException('Vocẽ não tem permissão para cancelar uma requisição que não te pertence.');
    }
    ...
    if (sol.status !== StatusSolicitacao.PENDENTE) {
      throw new BadRequestException('Não é possível cancelar. Tente contatar o RH caso já esteja aprovado.');
    }
    ...
    if (updErr) throw new BadRequestException('Não foi possível cancelar a solicitação.');
    ...
    await supabase.from('audit_log').insert({
      ...
      acao: 'CANCELAMENTO',
      usuario_id: userId,
      dados: { acao: 'CANCELADO_PELO_SERVIDOR', isOwer: sol.solicitado_por === userId },
